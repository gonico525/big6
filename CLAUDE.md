...
**仕様書が正**: [`docs/spec/prisoner-training-app-spec-v0.11.md`](docs/spec/prisoner-training-app-spec-v0.11.md)。
...
```sh
npm test        # logic.js のテスト（node:test、依存なし）。40 ケース
...
- `fetch(url, { cache: 'no-cache' })` で取ったものだけはネットワーク優先で返る。
  「その場で最新が欲しい」要求（`standards.json` の読み込み）を次回起動まで待たせないため。
  逆に言うと、**通常の `fetch` は必ず古い方が返る**。新しい取得を足すときはどちらが要るか考える
